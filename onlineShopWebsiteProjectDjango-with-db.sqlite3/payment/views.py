...
#                 return HttpResponse('پرداخت شما با موفقیت ثبت شد. البته این تراکنش قبلا ثبت شده است.')
...
#     else:
#         return HttpResponse(f'تراکنش نا موفق بود.')
...
def payment_process(request):
    ...
    request_data = {
        'merchant_id': "f588be9a-5417-4c12-991e-b3aa81640516",
        'amount': rial_total_price,
        'description': f'#{order.id}: {order.user.first_name} {order.user.last_name}',
        'callback_url': request.build_absolute_uri(reverse('payment:payment_callback')),
    }
